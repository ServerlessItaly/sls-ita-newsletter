...
export class StateMachine extends Construct {
    ...
    constructor(scope: Construct, id: string, props: StateMachineProps) {
        ...
        emailConfirmationFunction.addToRolePolicy(
        ...
        const isValidRequest = sfn.Condition.or(
            sfn.Condition.and(
                ...
                sfn.Condition.isNotPresent('$.emailItem.Item')
            ),
            sfn.Condition.and(
                sfn.Condition.stringEquals('$.type', 'unsubscribe'),
                sfn.Condition.isPresent('$.emailItem.Item')
            )
        );
        ...
        this.stateMachine = new sfn.StateMachine(this, 'Newsletter', {
            definition: getEmail.next(
                new sfn.Choice(this, 'IsValidRequest?')
                    .when(
                        isValidRequest,
                        sendEmailConfirmation.next(
